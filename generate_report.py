...
def data_row(label, cur, lw, ly, name, accent, is_first):
    ...
    return (
        ...
        f'<div style="width:40%;padding-right:8px">{nm}<div style="font-size:9px;font-weight:600;letter-spacing:0.1em;'
        f'text-transform:uppercase;color:{lc}">{label}</div></div>'
...
def region_block(region, rrows):
    ...
    covers_header = "" if is_parm else (
        ...
        f'<div style="width:40%"><div style="font-size:10px;font-weight:700;letter-spacing:.1em;'
        f'text-transform:uppercase;color:#9ca3af">Covers</div></div>'
        f'<div style="flex:2;text-align:right;font-size:12px;font-weight:600;color:#374151;'
        f'font-variant-numeric:tabular-nums">{fmtC(tCC)}</div>'
    ...
    return (
        ...
        f'<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><div style="width:3px;height:14px;'
        f'border-radius:2px;background:{ac}"></div>'
